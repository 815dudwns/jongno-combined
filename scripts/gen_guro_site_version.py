#!/usr/bin/env python3
"""data/guro-site-data.json → data/guro-site-data.version.json 생성.
캐시-우선 로더(js/idb.js + map.js loadSiteDataCached)가 이 버전으로 폰 IndexedDB 캐시 무효화 판단.
★ guro-site-data.json 변경(데이터 수정/추가) 후 반드시 다시 실행할 것.
...
SRC = ROOT / "data" / "guro-site-data.json"
OUT = ROOT / "data" / "guro-site-data.version.json"
...
print(f"guro-site-data.version.json 생성: version={ver} count={count}")
